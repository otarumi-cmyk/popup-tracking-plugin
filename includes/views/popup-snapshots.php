...
<div class="wrap popup-tracking-admin">
    ...
    <div style="margin:20px 0;padding:20px;background:#fff;border:1px solid #ddd;border-radius:4px;">
        ...
        <div style="margin:15px 0;padding:15px;background:#f5f5f5;border-radius:4px;">
            ...
            <ul style="margin:10px 0 0 20px;">
                ...
                <li>閉じた数: <?php echo number_format($total_closes); ?></li>
                <li>CTR: <?php echo $total_ctr; ?>%</li>
            </ul>
        </div>
        
        <button type="button" id="save-popup-snapshot-btn" class="button button-primary" style="font-size:14px;padding:8px 20px;margin-top:10px;">
            📸 現在のバナー設定を記録
        </button>
    </div>
    ...
    <div class="snapshots-section" style="margin-top:40px;padding:20px;background:#fff;border:1px solid #ddd;border-radius:4px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px;">
            ...
            <?php if (!empty($snapshots)) : ?>
            <button type="button" id="delete-all-popup-snapshots-btn" class="button" style="background:#e74c3c;color:#fff;border:none;">
                🗑️ すべての記録を削除
            </button>
            <?php endif; ?>
        </div>
        ...
        <table class="wp-list-table widefat fixed striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo number_format($snapshot['closes'] ?? 0); ?></td>
                    <td><strong style="color:#2ed573;"><?php echo $snapshot_ctr; ?>%</strong></td>
                    <td>
                        <?php 
                        $cta_info = array();
                        if (!empty($snapshot['cta_id'])) {
                            $cta_info[] = 'CTA: ' . esc_html($snapshot['cta_id']);
                        }
                        if (!empty($snapshot['variant'])) {
                            $cta_info[] = 'バリアント: ' . esc_html($snapshot['variant']);
                        }
                        if (!empty($snapshot['image_url'])) {
                            $cta_info[] = '<a href="' . esc_url($snapshot['image_url']) . '" target="_blank">画像</a>';
                        }
                        if (!empty($snapshot['abtest_enabled'])) {
                            $cta_info[] = 'A/Bテスト: 有効';
                        }
                        echo !empty($cta_info) ? implode('<br>', $cta_info) : '-';
                        ?>
                    </td>
                    <td>
                        <button type="button" class="button delete-snapshot-btn" data-index="<?php echo $index; ?>" style="padding:2px 8px;font-size:12px;">削除</button>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    <!-- 記録モーダル -->
    <div id="popup-snapshot-modal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.7);z-index:100000;align-items:center;justify-content:center;">
        <div style="background:#fff;padding:30px;border-radius:8px;max-width:500px;width:90%;">
            ...
            <table class="form-table">
                <tr>
                    <th><label for="snapshot-name">記録名</label></th>
                    <td>
                        <input type="text" id="snapshot-name" class="regular-text" placeholder="例: クリスマスキャンペーン" value="">
            ...
            <div style="margin-top:20px;text-align:right;">
                <button type="button" id="cancel-snapshot-btn" class="button">キャンセル</button>
                <button type="button" id="confirm-save-snapshot-btn" class="button button-primary">記録する</button>
            </div>
        </div>
    </div>
    
    <!-- データ属性で統計情報を渡す -->
    <div id="popup-snapshot-data" 
